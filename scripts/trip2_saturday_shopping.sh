#!/usr/bin/env bash
# =============================================================================
# Trip 2 — Saturday Shopping in Ulm / Neu-Ulm
#
# Simulates a user planning a full Saturday shopping trip.
#   Start     : Ulm HBF (lat: 48.399416, lon: 9.982603)
#   End       : none (open-ended trip)
#   Transport : walk + publicTransport
#   Activities:
#     1. Shopping in Ulm    — 4 h, around Hirschstraße (lat: 48.398256, lon: 9.987297)
#     2. Eat (anywhere)     — 30 min, no location constraint
#     3. Shopping Neu-Ulm   — 4 h, area (lat: 48.395349, lon: 10.000520)
#
# Usage:
#   ./trip2_saturday_shopping.sh [BASE_URL]
#   QUESTMAP_API_URL=http://localhost:8000 ./trip2_saturday_shopping.sh
#
# Requires: curl, jq
# =============================================================================
...
DRAFT_ID="draft-shopping-$(date +%s)"
...
log "Starting trip-planning session  (Trip 2 — Saturday Shopping Ulm/Neu-Ulm)"
...
SESSION_RESPONSE=$(curl -sf -X POST "${BASE_URL}/trip-planning/sessions" \
  ...
  -d "$(jq -n \
    ...
    '{
      draftId: $draftId,
      startLocation: {lat: 48.399416, lon: 9.982603, label: "Ulm HBF"},
      transportModes: ["walk", "publicTransport"],
      steps: [
        {
          id: "step-shopping-ulm",
          type: "shop",
          title: "Shopping in Ulm",
          details: "Shopping around the Hirschstrasse pedestrian zone",
          time: {durationMinutes: 240},
          location: {
            type: "areaCircle",
            center: {lat: 48.398256, lon: 9.987297, label: "Hirschstrasse Ulm"},
            radiusMeters: 500
          },
          iconKey: "shopping_bag",
          colorValue: 4281730550
        },
        {
          id: "step-eat",
          type: "eat",
          title: "Eat",
          details: "Lunch or dinner, anywhere in Ulm or Neu-Ulm",
        ...
        {
          id: "step-shopping-neu-ulm",
          type: "shop",
          title: "Shopping in Neu-Ulm",
          details: "Shopping in the Neu-Ulm area",
          time: {durationMinutes: 240},
          location: {
            type: "areaCircle",
            center: {lat: 48.395349, lon: 10.000520, label: "Neu-Ulm shopping area"},
            radiusMeters: 600
          },
          iconKey: "shopping_bag",
          colorValue: 4281730550
...
post_answer() {
  local QUESTION_ID="$1"
  local VALUE_JSON="$2"  # raw JSON value (e.g. true, 30, "opt-id")
...
# Auto-answer logic for Trip 2 (Saturday shopping):
#   yesNo       → true  (accept transit, route suggestions, etc.)
#   number      → 30    (sensible duration in minutes as fallback)
#   text        → "casual, mixed shopping and food"
#   selection   → first option id (pick first offered option)
...
handle_question() {
  ...
  case "${KIND}" in
    ...
    number)
      ...
      ;;
    text)
      ANSWER_VALUE='"casual, mixed shopping and food"'
      ;;
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "  TRIP 2 — SATURDAY SHOPPING  (Ulm → Neu-Ulm)"
